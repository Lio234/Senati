...
CREATE PROCEDURE sp_registrar_departamento(
    IN d_id_departamento CHAR(5),
    IN d_departamento VARCHAR(30)
)
BEGIN
    INSERT INTO tb_departamento (id_departamento,departamento)
    VALUES (d_id_departamento, d_departamento);
END $$
DELIMITER;
...
DELIMITER $$
CREATE PROCEDURE sp_listar_departamentos()   BEGIN
...
DELIMITER $$
CREATE PROCEDURE sp_buscar_departamento_por_id(IN d_id_departamento char(5))
BEGIN
    SELECT *
...
-- Mostrar la departamento por id
DELIMITER $$
CREATE PROCEDURE sp_mostrar_departamento_por_id(
    IN d_id_departamento CHAR(5)
)
BEGIN
Select
...
CREATE PROCEDURE sp_filtrar_departamento(
    IN d_nombre_departamento VARCHAR(30)
)
BEGIN
    SELECT *
    FROM tb_departamento
    WHERE departamento LIKE CONCAT('%', d_nombre_departamento, '%');
END $$
DELIMITER ;
...
BEGIN
    ...
    SET departamento = d_departamento
    WHERE id_departamento = d_id_departamento;
END $$
DELIMITER ;
...
CREATE PROCEDURE sp_eliminar_departamento(IN p_id_departamento CHAR(5))
BEGIN
    -- Verificar si hay provincias asociadas con el departamento
    DECLARE num_provincias INT;
    SELECT COUNT(*) INTO num_provincias
    FROM tb_provincia
    WHERE provincia_id_departamento = p_id_departamento;

    IF num_provincias = 0 THEN
        -- Si no hay provincias asociadas, proceder a eliminar el departamento
        DELETE FROM tb_departamento
        WHERE id_departamento = p_id_departamento;
        
        SELECT 'Departamento eliminado exitosamente.' AS mensaje;
    ELSE
        -- Si hay provincias asociadas, no se elimina y se devuelve un mensaje
        SELECT 'No se puede eliminar el departamento. Tiene provincias asociadas.' AS mensaje;
    END IF;
END //
